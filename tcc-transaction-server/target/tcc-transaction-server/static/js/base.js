jQuery(document).ready(function($) {
	...
	$('.table > tbody').on('click', '.j-edit', function () {
		...
		var $this = $(this);
		var globalTxId = $this.parent().siblings().eq(1).text();
		var branchQualifier = $this.parent().siblings().eq(2).text();
		var domain = (location.href.match(/domain=([^&]+)/) || [,''])[1];
		if(!domain) {
			return;
		}

		var url = 'management/retry/reset';
		$.ajax({
			url: url,
			type: 'PUT',
			dataType: 'json',
